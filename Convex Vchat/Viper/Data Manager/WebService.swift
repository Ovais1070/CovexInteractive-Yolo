...
class Webservice : PostWebServiceProtocol {
    ...
    fileprivate func send(_ response: (DataResponse<Any>)?) {
        ...
        if response.response?.statusCode == StatusCode.unAuthorized.rawValue  { // Validation For UnAuthorized Login
            ...
            do{
                ...
                print("JSONSerializationdata",json ?? "JSONSerializationdata")
    ...
    func send(api: Base, url appendingUrl : String?, data: Data?, parameters : [String : Any]?, type : HttpType) {
        ...
        print("At Webservice Request Url" , url)
...
//
        guard url != nil else { // Flow validation in url request
            ...
            //   print("At Webservice parameters  ", parameters!)

        
        Alamofire.request(url ?? "", method: .post, parameters: parameters, headers: nil).validate(statusCode: StatusCode.success.rawValue..<StatusCode.multipleResponse.rawValue).responseJSON { (response:DataResponse<Any>) in
    ...
    //    Alamofire.request(urlRequest!).validate(statusCode: StatusCode.success.rawValue..<StatusCode.multipleResponse.rawValue).responseJSON { (response) in
            ...
            case .success(let val):
                print("At Webservice Response new ",api,"",val, response.response?.statusCode ?? 0)
                
                 
 
                   if  let data = response.result.value as? NSDictionary
                   {
                     print("At Webservice Response data " , data)
                    if  let status = data.object(forKey: "code") as? Int
                    {
                        print("At Webservice Response status " , status)
...
//        headers.updateValue(WebConstants.string.bearer+String.removeNil(User.main.accessToken), forKey: WebConstants.string.Authorization)
        ...
        Alamofire.upload(multipartFormData: { (multipartFormData) in
            ...
            case .success(let upload, _, _):
                upload.responseJSON(completionHandler: { (response) in
                    ...
                    print("Print in upload: \(response)")
    ...
    private func sendImage(api: Base, imageData: [String:Data]?, parameters: [String : Any]?){
