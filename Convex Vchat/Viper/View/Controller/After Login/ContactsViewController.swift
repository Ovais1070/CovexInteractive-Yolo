...
class ContactsViewController: UITableViewController, RefreshContacts {
...
//        vc.delegate = self
 ...
 tableView.register(UINib(nibName: "ContactCell", bundle: nil), forCellReuseIdentifier: "ContactCell")
    ...
    func MakeVideoCall(index : Int){
        ...
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "RingingVideoCallVC") as! RingingVideoCallVC
                ...
                do {
                    try store.enumerateContacts(with: request, usingBlock: { (contact, stopPointer) in
                       ...
                       // print(contact.phoneNumbers.first?.value.stringValue)
                        ...
                        var imgData = Data()
                       // print("image", imgData)
                       // print("User Num", contact.phoneNumbers.first?.value.stringValue )

//                       let data = UserContactInfo(belongs_to: 0, blocked: 0, created_at: "", deviceId: "", exists: 0, host: "", isOnline: 0, is_blocked: 0, mood: "", name: contact.givenName, number: (contact.phoneNumbers.first?.value.stringValue as! NSString).integerValue , profile_pic: "", username: "")
//
//
//                        self.contactInfo.append(data)
                        
                        
                      //  print("Contacts Count", data.name)
...
//
                   ...
                   // print("Contacts Count", contact.givenName)
                         self.sendcontacts()
                        self.getContacts()
